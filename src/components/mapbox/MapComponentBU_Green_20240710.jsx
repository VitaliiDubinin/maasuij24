...
import { clearSelection } from '../../mapSlice';
...
//  const clonedPointRef = useRef(clonedPoint);
  ...
  useEffect(() => {
    ...
    mapRef.current.on('load', () => {
      ...
      const pointsSource = mapRef.current.getSource('points');
      console.log("Points Source Data:", pointsSource._data);
      // mapRef.current.on('sourcedata', (e) => {
      //   if (e.sourceId === 'points') {
      //     const updatedPointsSource = mapRef.current.getSource('points');
      //     console.log("Updated Points Source Data:", updatedPointsSource._data);
      //   }
      // });
      ...
      const clonedPointsSource = mapRef.current.getSource('cloned-points');
      console.log("ClonedPoints Source Data:", clonedPointsSource._data);
      mapRef.current.addLayer({
        id: 'cloned-points',
        type: 'circle',
      ...
      // console.log("ClonedPoints Source Data:", clonedPointsSource._data);
      // mapRef.current.on('sourcedata', (e) => {
      //   if (e.sourceId === 'cloned-points') {
      //     const updatedClonedPointsSource = mapRef.current.getSource('cloned-points');
      //     console.log("Updated ClonedPoints Source Data:", updatedClonedPointsSource._data);
      //   }
      // });
...
//   };
  ...
  const onClonedPointDrop = async () => {
     console.log("fron onClonedPointDrop")
    if (!clonedPoint) return;
  console.log("from onClonedPointDrop2")
    const updatedCoordinates = clonedPoint.coordinates;
    const originalPointId = clonedPoint.id;
  
    const originalPoint = pointsData.features.find(point => point.id === originalPointId);
